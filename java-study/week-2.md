...
## 5. 형 변환(Casting)
**형 변환이란, 변수 또는 상수의 타입을 다른 타입으로 변환하는 것**을 말합니다. 영어로는 type casting이라고 합니다. 타입은 자동으로 변환되기도 하고 수동으로 개발자가 변환할 수도 있습니다. 형변환 방법은 아래와 같습니다.

> **(타입) 피연산자**

정말 간단합니다. 단지 형변환을 하고 싶은 변수나 리터럴 앞에 변환하고자 하는 타입을 소괄호 안에 명시하기만 하면 됩니다. 여기서 사용되는 소괄호를 '**형변환 연산자**'라고 부릅니다.

### 5-1. 캐스팅(Casting)과 타입 프로모션(Type promotion)
...
이렇기 때문에, 22억은 값의 유실 없이는 int형에 담을 수 없기 때문에 컴파일러가 필요하면 형변환을 하라고 컴파일 에러로 개발자에게 알려주는 것입니다. 영문도 모르게 자동형 변환이 되어 값이 유실되면 프로그래밍 연산에서 치명적인 오류를 발생시킬 수 있기 때문이죠. **이렇게 더 큰 타입을 보다 작은 타입으로 형변환하는 것을 타입 캐스팅(Type casting)이라고 합니다.** 그렇다면 이젠 22억을 강제로 형변환을 한 후의 값을 확인해보겠습니다.
...
d의 출력값은 위처럼 이상한 음수값이 나옵니다. 어떻게된 일일까요?? 아까 22억을 2진수로 변환하면 총 32자리고 21억을 변환하면 31자리라고 했습니다. 여기서 22억이 할당된 c를 int로 형변환한다면 (값이 짤린다는 설명 추가)

그러면 다시 long 타입의 20,000을 int로 강제 형변환해보겠습니다.
...
### 5-3. 자동 형 변환시 규칙
형 변환이 필요한 이유는 타입이 다른 두 변수를 연산하기 위해서입니다. 그렇다면 컴파일러가 자동 형 변환을 해줄 때 어떤 판단 기준으로 형 변환을 할까요? **컴파일러는 기존의 값을 최대한 보존할 수 있는 타입으로 자동 형 변환을 합니다.**
...
### 6-1. 2차원 배열 만들기
위에서 만들어본 배열을 1차원 배열이라고 하며, 자바에서는 2차원, 3차원 배열 등 다차원 배열도 제공합니다. 여기선 2차원 배열까지 알아보겠습니다.

```java
int[][] numbers = new int[3][4];
```

1차원 배열과는 다르게 대괄호가 하나씩 더 추가되었습니다. numbers라는 이름으로 스택 메모리에 변수가 생성되었으며, 이 변수는 실제 값들이 저장되는 힙 메모리의 저장공간의 주소를 갖고 있습니다.(0x01) 그리고 눈여겨 봐야할 곳이 [3][4]부분입니다. 아래 메모리 그림을 보겠습니다.

<img width="640" alt="스크린샷 2021-09-07 오후 8 37 20" src="https://user-images.githubusercontent.com/80696862/132338264-cfbcc648-7d5e-4c7e-af98-2be916a95626.png">

배열 생성 코드에서 new int[3][4]는 해석하자면 길이가 3인 1차원 배열의 각 요소들에는 실게 값이 아닌 길이가 4인 배열의 주소값이 저장되어있습니다. 위 그림처럼 길이가 3인 배열의 저장공간이 Heap영역에 생겼으며, 이 배열의 각 속성들에는 주황색의 길이가 4인 배열의 주소값이 들어있습니다. 실제 값들은 모두 이 길이가 4인 배열에 저장됩니다. 그러면 값을 할당하는 방법을 알아보겠습니다.

```java
numbers[0][0] = 10; //0번째 인덱스에 저장된 배열의 0번째 인덱스
numbers[0][1] = 20; //0번째 인덱스에 저장된 배열의 1번째 인덱스
numbers[0][2] = 30;
numbers[0][3] = 40;
...
<img width="640" alt="스크린샷 2021-09-07 오후 8 49 33" src="https://user-images.githubusercontent.com/80696862/132339751-752224b8-6c8f-4a76-8ffd-9a9047cecf3a.png">
...
## 7. 타입 추론, var
타입 추론이란 코드에 변수의 타입을 명시하지 않아도 컴파일러가 그 타입을 유추할 수 있는 것을 말합니다. 자바 10버전부터 지원되고 있습니다. 타입 추론을 사용해 타입 명시 없이 변수를 생성할 수 있습니다.
...
문자열 s1을 선언하고 s1을 var로 받은 s2에 할당합니다. 그리고 아래의 출력문을 활용하여 s2가 String인지 아닌지 확인할 수 있습니다.

<img width="400" alt="스크린샷 2021-09-07 오후 10 11 26" src="https://user-images.githubusercontent.com/80696862/132350551-5a22d917-28c8-4bcf-958b-d1d8a19555ec.png">
