...
## 5. 형변환(Casting)
**형변환이란, 변수 또는 상수의 타입을 다른 타입으로 변환하는 것**을 말합니다. 영오른 type casting이라고도 합니다. 이 타입은 자동으로 변환되기도 하고 수동으로 개발자가 변환할 수도 있습니다. 형변환 방법은 아래와 같습니다.
...
네. 진짜로 붙여줍니다. int의 메모리 저장공간은 4byte고 long의 메모리 저장공간은 8byte입니다. 작은 공간에 있는 값을 더 큰 공간으로 옮기는 것이니 값이 유실되거나 변경될 우려가 전혀 없습니다. 따라서 컴파일러는 타입 연산자가 명시되어있지 않아도 알아서 컴파일시 타입 연산자를 만들어 붙여줍니다. 반대의 경우엔 어떨까요?
...
<img width="550" alt="스크린샷 2021-09-05 오전 4 45 44" src="https://user-images.githubusercontent.com/80696862/132106312-367ad84d-c183-4ac0-996f-5b515d1bf8bf.png">

파란색의 scores 메모리 공간 내부엔 저렇게 인덱스 0번부터 4번까지의 방이 생긴 것을 확인할 수 있습니다. 아직 우리가 배열을 생성만 했지 실제 배열의 각 방에 값을 할당하지 않았기 때문에 각 방엔 int타입의 초기값인 0으로 초기화 되어있습니다. 이번엔 배열에 값을 할당하는 방법을 알아보겠습니다.
