...
모든 자바 애플리케이션은 메인 스레드가 메인 메소드를 실행하면서 시작됩니다. 스택이라는 이럼처럼 LIFO 구조로 메소드가 호출될때마다 스택 프레임이 하나씩 쌓입니다. 이 스택 프레임 하나엔 로컬 변수들이 선언되며, 해당 스택 프레임과 생명주기를 같이합니다.

<img width="700" alt="스크린샷 2021-10-09 오후 9 13 59" src="https://user-images.githubusercontent.com/80696862/136657430-f20a8f1b-8ecd-458a-9875-57bbfac42205.png">
...
메인 메소드가 실행되면, 메인 스레드 내부에 메인 스레드 스택이 하나 쌓입니다. 이후 메인 메소드에서 다른 메소드를 호출할때마다 스레드 스택이 하나씩 쌓입니다.
