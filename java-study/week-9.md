...
# 4. 사용자 정의 예외
자바의 예외 상속 계층을 살펴본 것처럼, 자바에서는 이미 수 많은 종류의 예외를 제공하고 있습니다. 하지만 이외에도 사용자 정의 예외를 만들어서 사용할 수 있습니다. 새로운 예외를 만들떈 내가 만들고자 하는 예외의 종류(Checked Exception, Unchecked Exception) 중 하나의 타입을 선택해 상속해서 만듭니다. 오라클에서는 아래의 기준중 1개 이상 충족하면 사용자 정의 예외를 만들 것을 권하고 있습니다.

- 자바에서 제공되지 않는 예외 타입이 필요한지?
- 다른 벤더에서 제공하는 클래스에서 발생한 예외와 사용자 정의 예외를 구별할 수 있다면 사용자에게 도움이 되는지?
